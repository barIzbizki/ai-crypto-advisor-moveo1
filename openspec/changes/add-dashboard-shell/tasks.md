...
- [ ] 1.1 Add a small shared helper to read the authenticated user's saved preferences row (`assetsOfInterest`, `investorType`, `contentTypes`) by `user_id`, reused by the news/prices/insight routes. (`CRYPTOPANIC_API_KEY`/`CRYPTOPANIC_API_BASE_URL` are added by `add-market-news`; `COINGECKO_API_BASE_URL` is added by `add-coin-prices`; `LLM_API_KEY`/`LLM_API_BASE_URL` are added by `add-ai-insight`.)
...
## 4. AI Insight of the Day section (PM-16, delivered by the `add-ai-insight` change)

- [ ] 4.1 Mount `AiInsightSection` (from `add-ai-insight`, `frontend/src/components/dashboard/AiInsightSection.jsx`) in the AI Insight of the Day slot; no separate backend work here — `GET /dashboard/insight` and its Hugging Face integration/fallback are owned by `add-ai-insight`
...
- [ ] 6.2 Replace `frontend/src/pages/DashboardPage.jsx`'s health-check body with 4 section components rendered in order: Market News (`MarketNewsSection` from `add-market-news`, task 2.1), Coin Prices (`CoinPricesSection` from `add-coin-prices`, task 3.1), AI Insight of the Day (`AiInsightSection` from `add-ai-insight`, task 4.1), Fun Crypto Meme
