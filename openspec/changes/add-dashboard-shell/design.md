...
- No outbound HTTP-to-third-party-API calls exist anywhere in the backend yet — this change introduces the first ones (CryptoPanic, CoinGecko, an LLM provider).
...
- **Fallback behavior differs by section, matching what makes sense for that data:** Market News and AI Insight fall back to static/generic content (a missing news feed or LLM response is not user-facing-critical); Coin Prices surfaces an explicit error state instead of fabricating numbers (stale/fake prices would be actively misleading); Fun Crypto Meme has no external dependency at all in this change (ships as a small static/curated set) to avoid depending on a third API on day one — swapping in a live meme API is a future enhancement, not required by PM-37.
- **New third-party integrations are isolated behind small internal modules (e.g. `backend/src/services/cryptopanic.js`, `coingecko.js`, `llmInsight.js`), each owning its own fetch + fallback logic**, so the route handlers stay thin and each integration can be unit-tested by mocking its module.
- **New env vars follow the existing `loadConfig()` pattern**: `CRYPTOPANIC_API_KEY`, `COINGECKO_API_BASE_URL` (CoinGecko's free tier needs no key), `LLM_API_KEY`/`LLM_API_BASE_URL` (e.g. Hugging Face Inference API) — all optional in development (missing key/url triggers that section's fallback path rather than throwing at startup), required only where the project's deployment config decides.

## Risks / Trade-offs

- [Free-tier third-party APIs (CryptoPanic, CoinGecko, a free LLM provider) have rate limits and no uptime SLA] → Acceptable for this change: each section has a fallback path, and this is a shell/MVP dashboard, not a trading tool.
...
1. Add backend service modules for each external integration (`cryptopanic.js`, `coingecko.js`, `llmInsight.js`), each exporting a function that takes the user's relevant preference field(s) and returns section data or throws/returns a fallback marker.
