...
Logged-in users land on a `/dashboard` page that only shows a backend health check — it has no real content. This change implements Jira story [PM-13] "Dashboard shell", covering its subtask [PM-37] (dashboard page rendering 4 fixed sections, using saved preferences to filter/tailor content). Market News (PM-14), Coin Prices (PM-15), and AI Insight of the Day (PM-16) are delivered by sibling changes and composed here rather than re-implemented.
...
- **Backend**: add a new route module (e.g. `backend/src/routes/dashboard.js`) exposing `GET /dashboard/meme` behind `requireAuth` for Fun Crypto Meme, reading the user's preferences via the existing `preferences` table/query where applicable; wire into `backend/src/routes/index.js`. Market News' `GET /dashboard/news` endpoint is added by `add-market-news`, Coin Prices' `GET /dashboard/prices` endpoint is added by `add-coin-prices`, and AI Insight's `GET /dashboard/insight` endpoint (plus its Hugging Face Inference API dependency and `LLM_API_KEY`/`LLM_API_BASE_URL` config) is added by `add-ai-insight` — none of these are built here.
- **Frontend**: replace `frontend/src/pages/DashboardPage.jsx`'s health-check placeholder with 4 section components in order (Market News via `add-market-news`'s `MarketNewsSection`, Coin Prices via `add-coin-prices`'s `CoinPricesSection`, AI Insight of the Day via `add-ai-insight`'s `AiInsightSection`, plus Fun Crypto Meme built here), each fetching its own data via `apiFetch` and handling independent loading/error/empty states.
